...
export const register = (app: express.Application) => {
    ...
    const port = parseInt ( process.env.PGPORT || "8081", 10 );
    const config = {
    ...
    app.get( `api/guitars/find/:search`, oidc.ensureAuthenticated(), async ( req: any, res ) => {
        try {
            ...
            FROM guitars 
            WHERE user_id = $[userId] 
            AND (brand LIKE $[search] OR model LIKE $[search] OR year LIKE $[search] OR color LIKE $[search]) 
    ...
    app.post( `api/guitars/add`, oidc.ensureAuthenticated(), async ( req: any, res ) => {
        try {
            ...
            INSERT INTO guitars (brand, model, year, color, user_id) 
            VALUES ($[brand], $[model], $[year], $[color], $[userId]) 
            RETURNING id`, 
            { userId, ...req.body } );
            return res.json( {id} );
        } catch ( err ) {
    ...
    app.delete( `api/guitars/delete/:id`, oidc.ensureAuthenticated(), async ( req: any, res ) => {
        try {
            const userId = req.userContext.userinfo.sub;
